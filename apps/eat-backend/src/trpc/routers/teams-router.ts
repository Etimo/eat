  ...
  create: protectedProcedure
    ...
    .query(async ({ ctx}) => {
      ...
      const teams = await db.teams.findAll({
        ...
        populate: ['teamMemberships', 'teamMemberships.user', 'teamMemberships.user.activities'],
      });
      ...
      const teamsWithActivities = teams.map(team => {
        ...
        const activitiesInCompetition = team.teamMemberships.map(({ user }) => 
          user.activities.filter(activity => activity.competition.id === currentCompetition.id)
        );
